...
(function () {
  ...
  const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = root + 'styles.css'; document.head.appendChild(link);

  const ready = (async () => {
    const theme = await (await fetch(root + 'tokens/tailwind.css')).text();
    ...
    const comps = {};
    await Promise.all(names.map(async (n) => { comps[n] = await loadModule(root + 'components/vue/' + n + '.vue', opts); }));
    return comps;
  })();
