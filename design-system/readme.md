...
```vue
<script setup>
import { P5Button, P5Heading, P5Card } from './design-system/components/vue'
</script>
<template>
...
## Tokens
`styles.css` → `tokens/*.css` (custom properties en `:root`). Prefijos: `--p5-*` color base, `--bg/--fg/--accent` semánticos, `--type-*` familias, `--size-*` tamaños, `--track-*`, `--lh-*`, `--hard-*` sombras, `--skew-*`, `--rot-*`, `--clip-*`, `--easing-*`, `--dur-*`, `--space-*`, `--z-*`.
`tokens/tailwind.css` mapea esos tokens a utilidades (`bg-ink`, `text-paper`, `shadow-hard-lg`, `text-h1`, `font-display`, `ease-pop`, `animate-stamp`) y añade utilidades propias: `skew-p5[-xs|-md|-lg]`, `unskew-p5`, `clip-slant`, `text-shadow-hard[-lg]`, `text-shadow-ink`, `text-stroke-fg`, `halftone-ink|paper`, `zigzag`, `stripes-red`, `label-p5`, `display-p5`, `pop-hover`.

## Componentes (Vue 3, `components/vue/`)
...
## Índice
- `styles.css`, `tokens/` — tokens CSS y `tailwind.css` (entrada Tailwind 4)
- `components/vue/` — SFC + `index.js` (barril) · `components/cards/` — specimens
...
## Caveats
- Fuentes vía Google Fonts (sin binarios locales).
