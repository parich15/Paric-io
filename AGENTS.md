...
## Guías obligatorias
Leer completos [DESIGN.md](DESIGN.md) y [DESIGN-SPECS.md](DESIGN-SPECS.md) antes de tocar UI. Jerarquía: decisiones del usuario y excepciones documentadas; DESIGN para valores; DESIGN-SPECS para intención; [prototipos](references/) para composición/interacción. La fuente única de tokens es [design-system/tokens](design-system/tokens/), mapeada en `tailwind.css`. Componentes P5 en `design-system/components/vue`; specimens y guidelines en `design-system`. No copiar los tokens a las skills.

Las rutas de componentes y tokens de las guías originales son relativas a `design-system/`. Sus instrucciones de demo/CDN no sustituyen convenciones Nuxt de producción. Las fuentes ahora se sirven localmente desde `public/fonts` con sus licencias. Leer el [ADR de contraste](docs/adr/001-contraste.md): conservar colores, tamaños y placas originales. Contacto es una maqueta sin envío; los proyectos son demos sin resultados reales acreditados.
