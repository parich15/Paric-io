...
test('el formulario redacta un mailto con el borrador y el sitio nunca envía nada', async ({ page }) => {
  ...
  await expect(address).toHaveAttribute('href', 'mailto:oscar@oscarparic.io')
...
test.describe('contacto sin JavaScript', () => {
  ...
  for (const locale of ['es', 'en'] as const) {
    test(`HTML completo en ${locale}`, async ({ page }) => {
      ...
      await expect(page.locator('form[action="mailto:oscar@oscarparic.io"][method="post"][enctype="text/plain"]')).toHaveCount(1)
