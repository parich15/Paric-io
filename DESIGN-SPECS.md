...
También por petición del usuario (septiembre de 2026), dos secciones de la portada toman la narrativa del juego además de su gramática. **Obras** es un plano de «ruta de infiltración»: una hoja de papel con manzanas, un plan discontinuo y una ruta roja con sombra de tinta que el scroll dibuja (`onScroll` sincronizado de Anime.js) hasta pasar por las tres paradas destacadas y sellar un objetivo con el total de proyectos. La lista de paradas, enlazada a cada detalle, es la navegación real; el plano es decorativo. **Sobre mí** es una carta de confidente: carta de tarot con el retrato (arcano I, «Le Bateleur»), identidad sobre una banda roja y seis habilidades elegidas con estrellas. Cada estrella es un radio nativo y la placa visible se resuelve con CSS, así que sin JavaScript también se cambia de habilidad; el script solo añade la subida de rango. El rango es la posición de la habilidad en la carta, no un nivel. Sin JavaScript o con movimiento reducido, el plano aparece ya dibujado y la carta, repartida.
